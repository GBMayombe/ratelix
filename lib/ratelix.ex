defmodule Ratelix do
  ...
  ## Example
      ...
      {:ok, _pid} = Ratelix.start_link(:my_leaky, algorithm: :leaky_bucket, capacity: 5, interval: 1)
      Ratelix.wait_for_turn(:my_leaky)

      # Start a token bucket limiter
      {:ok, _pid} = Ratelix.start_link(:my_token, algorithm: :token_bucket, capacity: 10, rate: 2, interval: 1)
      Ratelix.wait_for_turn(:my_token)
  """
  ...
  """
  @spec start_process(name(), options()) :: DynamicSupervisor.on_start_child()
  defdelegate start_process(name, opts), to: DynamicSup
  ...
  defp execute(name, func) do
    case Ratelix.Registry.lookup(name) do
      nil -> {:error, :process_not_found}
      mod -> apply(mod, func, [name])
    end
  end
end
